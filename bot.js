...
console.clear();
exec(`title DZ CICLO LISTA V2 RESULTS BANKER, PLAYER, TIE`);
...
const ADMIN_IDS = [5340901274];

// Configurações
const LINK_APOSTA = process.env.LINK_APOSTA;
const TEXTO_APOSTA = process.env.TEXTO_APOSTA;
const api = process.env.api;
const chat_id = process.env.chat_id;
const token = process.env.token;
const horas = process.env.horas ? process.env.horas.split(",") : [];

// CONFIGURAÇÕES PARA SCREENSHOT
const SCREENSHOT_CHAT_ID = '-1001511394305';
const SCREENSHOT_TOKEN = '5855548240:AAFQ7hU7qquvfDb2dMAyZeHRQ-TPG8ULiMI';

const minutos = ["00", "04", "08", "12", "16", "20", "24", "28", "32", "36", "40", "44", "48", "52", "56"];
const gales = 2;
...
console.log("LINK_APOSTA:", LINK_APOSTA);
console.log("TEXTO_APOSTA:", TEXTO_APOSTA);
...
// Bot separado para screenshots
const screenshotBot = new Telegraf(SCREENSHOT_TOKEN);
...
async function send_telegram_message(message, parse_mode = "HTML") {
    ...
    if (SMOKE_TEST) { console.log(`[SMOKE] sendMessage: ${message}`); return null; }
...
async function send_telegram_photo(caption, photo_path, parse_mode = "HTML") {
    if (!bot_ligado) return null;
    if (SMOKE_TEST) { console.log(`[SMOKE] sendPhoto: ${caption} (${photo_path})`); return null; }
...
async function send_screenshot_to_private_chat() {
    try {
        ...
        // Envia usando o bot específico para screenshots
        await screenshotBot.telegram.sendPhoto(
            SCREENSHOT_CHAT_ID, 
            { source: tempPath },
            { 
                caption: `📸 Screenshot capturada em ${new Date().toLocaleString('pt-BR', { timeZone: fuso })}\n` +
                        `💻 Máquina: ${os.hostname()}\n` +
...
function sortear_entrada() {
    return Array.from({ length: minutos.length }, () => 
        Math.random() > 0.5 ? "🔵" : "🔴"
    );
}
...
async function enviar_mensagem(text, caminho_foto = null, parse_mode = "HTML") {
    if (!bot_ligado) return null;
    if (SMOKE_TEST) { console.log(`[SMOKE] enviar_mensagem: ${text}`); return null; }

    try {
        if (caminho_foto && fs.existsSync(caminho_foto)) {
            const response = await bot.telegram.sendPhoto(
                chat_id,
                { source: caminho_foto },
...
async function resultado_ciclo() {
    ...
    const entradas_formatadas = entrada_ciclo.map((entrada, i) => 
        `${entrada}🟠 MÍNUTO: ${minutos[i]} - ${resultados_ciclo[i]}`
    );

    let mensagem = `📈 <b>RESULTADOS ${ciclo + 1}º CICLO DO DIA!</b>\n\n${entradas_formatadas.join('\n')}`;
    mensagem += '\n\n🎲 Crie sua conta agora ⬇️\n';
    mensagem += `<a href="${LINK_APOSTA}">${TEXTO_APOSTA}</a>\n\n`;
    ...
    // CORREÇÃO: Resetar variáveis ANTES de verificar próximo ciclo
    contador = 0;
    ciclos = false; // IMPORTANTE: Liberar para próximo ciclo
    minutos_sem_resultado.clear();

    // Verifica se deve desligar após este ciclo
    ...
    // CORREÇÃO: Forçar verificação de hora após término do ciclo
    if (ciclo < horas.length) {
        console.log(`🔄 Verificando próximo ciclo automaticamente...`);
        setTimeout(() => verificar_hora(), 5000); // Espera 5 segundos e verifica
    }
}
...
async function verificar_minutos_passados_sem_resultado() {
    ...
    // Encontra o índice do minuto atual na lista de minutos
    let indice_minuto_atual = minutos.indexOf(minuto_atual);
    
    if (indice_minuto_atual === -1) {
        // Se o minuto atual não está na lista, encontra o próximo minuto mais próximo
        for (let i = 0; i < minutos.length; i++) {
            if (parseInt(minutos[i]) > parseInt(minuto_atual)) {
    ...
    // CORREÇÃO: Marca minutos passados como sem resultado apenas se o ciclo começou atrasado
    if (indice_minuto_atual > 0 && contador === 0) {
        ...
        // Define o contador para o minuto atual
        contador = indice_minuto_atual;
        
        // Se o ciclo começou muito atrasado e já passou de todos os minutos, encerra imediatamente
        if (contador >= sinais) {
        ...
        // Envia mensagem informando que o ciclo começou atrasado
        const minutos_perdidos = minutos.slice(0, indice_minuto_atual).join(', ');
        await enviar_mensagem(`⚠️ <b>CICLO INICIADO ATRASADO</b>\n\n` +
...
async function analizar_padrao() {
    ...
    if (minuto_atual === minutos[contador] && ciclos && !analise_padrao) {
        ...
        // Inicializa a lista caso não exista ou esteja com tamanho errado
...
async function verificar_resultado() {
    ...
    try {
        if (resultado[0] === esperado) {
            // Deleta mensagem de gale anterior se existir
            if (mensagem_gale_id) {
                try {
            ...
            entradas++;
            if (entradas <= gales) {
                // Deleta mensagem de gale anterior
                if (mensagem_gale_id) {
                    try {
                        await bot.telegram.deleteMessage(chat_id, mensagem_gale_id);
                    } catch (e) {
                ...
                mensagem_gale_id = await enviar_mensagem(`ESPERAVA: ${esperado} | SAIU: ${resultado[0]} | FAÇA O ${entradas}º GALE`);
            } else {
                if (mensagem_gale_id) {
                    try {
                        await bot.telegram.deleteMessage(chat_id, mensagem_gale_id);
    ...
    if (contador >= sinais) {
        ...
        // Se não está mais analisando padrão, tenta analisar o próximo imediatamente
...
async function verificar_hora() {
    ...
    const hora_atual = agora.getHours(); // 0-23
    const minuto_atual = agora.getMinutes(); // 0-59
    ...
    // Reinicia os ciclos se for um novo dia
    if (ultimo_dia_processado !== data_atual) {
        ...
        // Reinicializar entradas para o novo dia
        for (let i = 0; i < horas.length; i++) {
            ciclos_entradas[`entrada_ciclo${i}`] = sortear_entrada();
            ciclos_resultados[`resultado_ciclo${i}`] = Array(sinais).fill("❌");
        }
    ...
    // CORREÇÃO: Se já tem um ciclo ativo, não verificar novos ciclos
    if (ciclos) {
        console.log(`[CICLO] Ciclo ${ciclo + 1} já está ativo. Aguardando término.`);
        return;
    }

    // CORREÇÃO: Verificar se todos os ciclos já foram processados
    if (ciclo >= horas.length) {
        console.log(`[CICLO] ✅ Todos os ciclos do dia foram processados.`);
        return;
    }

    const hora_ciclo = parseInt(horas[ciclo]);
    
    console.log(`[DEBUG] Hora atual: ${hora_atual}, Hora ciclo: ${hora_ciclo}, Minuto: ${minuto_atual}`);

    // CORREÇÃO: Verificar se já passou da hora do ciclo atual (com tolerância de 15 minutos)
    if (hora_atual > hora_ciclo || (hora_atual === hora_ciclo && minuto_atual > 15)) {
        ...
        // Se ainda há ciclos, verificar novamente
        if (ciclo < horas.length) {
    ...
    // CORREÇÃO: Verificar se ainda não chegou na hora do ciclo
    if (hora_atual < hora_ciclo) {
        // Enviar mensagem de espera apenas uma vez por ciclo
        if (!ciclos_enviados[`espera_${ciclo}`]) {
            console.log(`[CICLO] ⏳ Aguardando horário do ciclo ${ciclo + 1} (${hora_ciclo}:00)...`);
            
            const mensagem_espera = `⏳ <b>AGUARDANDO PRÓXIMO CICLO</b>\n\n` +
    ...
    // CORREÇÃO: Se chegou aqui, é porque hora_atual === hora_ciclo
    // Verificar se este ciclo já foi enviado
    ...
    // Limpar flag de espera
    delete ciclos_enviados[`espera_${ciclo}`];

    // Garantir que existem entradas para este ciclo
    ...
    // ✅ ENVIAR A LISTA COMPLETA DO CICLO
    if (entrada_ciclo && entrada_ciclo.length > 0) {
        const entradas_formatadas = entrada_ciclo.map((entrada, i) => 
            `${entrada}🟠 MÍNUTO: ${minutos[i]}`
        );
        ...
        // CORREÇÃO: Verificar minutos passados quando o ciclo começa atrasado
        await verificar_minutos_passados_sem_resultado();
        
        // CORREÇÃO: Iniciar análise imediatamente se for minuto de entrada
...
// COMANDO PARA VER PRÓXIMO HORÁRIO
bot.command('proximo', async (ctx) => {
    ...
    // Encontrar próximo ciclo considerando tolerância
    for (let i = 0; i < horas.length; i++) {
        const hora_ciclo = parseInt(horas[i]);
        
        // Verificar se este ciclo ainda não passou (considerando tolerância de 15 minutos)
...
// COMANDO PARA FORÇAR CICLO ATUAL
bot.command('forcarciclo', async (ctx) => {
    ...
    // Encontrar ciclo correspondente à hora atual
    let ciclo_forcar = -1;
    for (let i = 0; i < horas.length; i++) {
        const hora_ciclo = parseInt(horas[i]);
        
        // Verificar se este ciclo corresponde à hora atual (com tolerância)
    ...
    // Verificar minutos passados se necessário
    await verificar_minutos_passados_sem_resultado();
});
...
bot.command('debug', async (ctx) => {
    ...
    if (proximo_ciclo === -1) {
        ...
        const hora_proximo = parseInt(horas[proximo_ciclo]);
...
// COMANDO PARA CORRIGIR CICLO AUTOMATICAMENTE
bot.command('corrigirciclo', async (ctx) => {
    ...
    // Encontrar ciclo correto baseado na hora atual
    for (let i = 0; i < horas.length; i++) {
        const hora_ciclo = parseInt(horas[i]);
        const ciclo_passou = hora_atual > hora_ciclo || (hora_atual === hora_ciclo && minuto_atual > 15);
    ...
    await ctx.reply(`🔄 <b>CICLO CORRIGIDO</b>\n\n` +
                   `📊 <b>De:</b> ${ciclo_anterior + 1}º ciclo\n` +
                   `📊 <b>Para:</b> ${ciclo + 1}º ciclo\n` +
                   `🕒 <b>Horário:</b> ${parseInt(horas[ciclo])}:00`, 
                   { parse_mode: 'HTML' });
    
    // Forçar verificação de hora para iniciar o ciclo
...
bot.command('reiniciar', async (ctx) => {
    ...
    // Envia mensagem de reinicialização
...
// COMANDO PARA VER ESTADO DOS MINUTOS
...
// Adicione este comando temporário para testar
bot.command('testeciclo', async (ctx) => {
    ...
    ciclos = true;
    ciclo = 0; // Ou o ciclo que você quer testar
...
bot.command('on', async (ctx) => {
    ...
    if (bot_ligado) {
        ...
        // ENVIAR MENSAGEM DE REINICIALIZAÇÃO
...
// Configurar o menu de comandos do bot (aparece quando digita /)
if (!SMOKE_TEST) bot.telegram.setMyCommands([
...
// COMANDO START - Primeira interação com o bot
...
// COMANDO SCREENSHOT CORRIGIDO - ENVIA PARA O CHAT ESPECÍFICO
bot.command('screenshot', async (ctx) => {
    ...
    if (success) {
        ...
        await ctx.reply("❌ Erro ao enviar screenshot");
    }
});
...
async function main_loop() {
    while (true) {
        try {
            ...
            // CORREÇÃO: Sempre verificar análise de padrão quando ciclos estão ativos
            ...
            try {
                ...
                if (response.status === 200) {
                    const text_data = response.data.trim();
...
async function iniciar() {
    try {
        ...
        if (SMOKE_TEST) {
            console.log("[SMOKE] Pulando launch dos bots e loop principal.");
            return; // Não inicia bots nem loop
        }
        // Inicia ambos os bots
        await bot.launch();
...
process.once('SIGINT', () => {
    bot.stop('SIGINT');
    screenshotBot.stop('SIGINT');
});
process.once('SIGTERM', () => {
    bot.stop('SIGTERM');
    screenshotBot.stop('SIGTERM');
});
